...
public class BoardEndpoints : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPost("/api/boards", async (
            ...
            .WithDescription(
                "Cria um novo board para o usuario autenticado. O OwnerId e obtido do token JWT, mesmo que venha no corpo da requisicao.")
            .WithTags("Boards")
        ...
        app.MapGet("/api/boards", async (
            ...
            .WithTags("Boards")
            .Produces<IEnumerable<BoardResponseDto>>()
            .Produces(401)
            .WithOpenApi();

        app.MapGet("/api/boards/{id:guid}",
                async (Guid id, IGetBoardByIdService getBoardByIdService, CancellationToken cancellationToken) =>
                {
        ...
        app.MapDelete("/api/boards/{id:guid}",
                async (Guid id, IDeleteBoardService deleteBoardService, CancellationToken cancellationToken) =>
                {

...
public class AuthEndpoints : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPost("/api/auth/login", async ([FromBody] LoginRequestDto request, ILoginService loginService,
            ...
            .WithDescription(
                "Autentica o usuário no sistema com base no Username e Password, retornando as informações do usuário e o token JWT correspondente.")
            .WithTags("Autenticação")
            .Produces<LoginResponseDto>()
            .Produces(401)
            .WithOpenApi();

        app.MapPost("/api/auth/register", async ([FromBody] CreateUserDto request,
                IRegisterUserService registerUserService, CancellationToken cancellationToken) =>
            {
        ...
        app.MapGet("/api/me",
                async (HttpContext httpContext, IGetUserByIdService getUserByIdService,
                    CancellationToken cancellationToken) =>
                {
